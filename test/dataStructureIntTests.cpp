// charVariety/unittest/dataStructureIntTests.cpp

#define BOOST_TEST_MODULE MyTest
#include <boost/test/included/unit_test.hpp> 
#include "../utils/dataStructureInt.h" 
namespace utf = boost::unit_test;

#define DELTA 1e-6

BOOST_AUTO_TEST_CASE(testSquareInt)
{
    // test for sqr
    Interval param{2, 3};
    Interval foundSquare = sqr(param);
    Interval expectedSquare{4, 9};
    BOOST_CHECK(expectedSquare == foundSquare);
}

BOOST_AUTO_TEST_CASE(testAbsInt)
{
    // test for abs
    Interval param = {-2, 1};
    Interval foundAbs = absVal(param);
    Interval expectedAbs = {0, 2};
    BOOST_CHECK(expectedAbs == foundAbs);
}

BOOST_AUTO_TEST_CASE(testCompMax) 
{
    // test for compmax
    Interval a = {2, 6}, b = {3, 4}, c = {5, 8};
    bool foundABC = compMax(a, b, c);
    bool foundBCA = compMax(b, c, a);
    bool foundCAB = compMax(c, a, b);
    bool expectedABC = 1, expectedBCA = 0, expectedCAB = 1;
